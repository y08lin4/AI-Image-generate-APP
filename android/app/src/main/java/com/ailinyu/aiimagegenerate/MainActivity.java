...
public class MainActivity extends BridgeActivity {
    ...
    private static final String WEB_FALLBACK_SCRIPT =
        "(function(){"
            ...
            + "    if(href.indexOf('data:image/')!==0)return;"
            + "    event.preventDefault();event.stopPropagation();"
            + "    try{"
            + "      var result=String(native.saveImage(href,target.download||'ai-image.png')||'');"
            + "      if(result.indexOf('error:')===0)alert('保存失败：'+result.replace(/^error:/,''));"
